...
function buildTemplate(brand, main, sub, hashtags) {
  const section1 = `<strong>📌 전반적인 가이드</strong>
...
`;
  ...
  // 사진 가이드 — 이미지 슬롯 전
  const section2a = `<strong>📸 사진 가이드</strong>

[촬영 방법]
(01) 쪽지로 보내드린 예시 화면을 그대로 캡처하지 마시고, <u>${brand} 서비스 소개서나 웹페이지의 다양한 화면을 컴퓨터, 휴대폰 화면으로 연 상태에서 촬영한 파일을 업로드 부탁 드립니다.</u>
(02) 원고 내에 사진들을 한 각도로(한 배경으로) 찍는 게 아닌 다양한 각도의 다른 배경으로 찍어서 작성 부탁 드립니다.`;

  // 사진 가이드 — 이미지 슬롯 후
  const section2b = `[첨부 및 배치 규칙]
(01) 사진 수 : 10 ~ 20장 이내로 첨부 부탁드립니다.
(02) 사진 활용 : [사진 1장] - [글 10줄 이내] - [사진 1장] - [글 10줄 이내] 의 형식으로 작성해주세요.
<span style="color:#E93535;font-weight:700;text-decoration:underline;">※ 사진 1장 당 사진 아래에 최소 글 4줄 ~ 5줄을 꼭 지켜주세요!</span>
(03) 사진이 3장 이상 모여있거나 연속되지 않게, 글은 10줄 이상 넘지 않도록 작성해주세요.
`;

  const section3 = `<strong>✒️ 내용 작성 가이드</strong>

[참고 자료]
(01) 보내드린 서비스 소개서 및 링크를 확인하여 작성하실 내용을 참고해주세요. (리뷰 선정대상자분들께 쪽지 드릴 예정입니다.)
...
예시) 브랜드 성공을 위한 첫걸음, ${brand}를 선택한 이유
1.~~
2.~~
3.~~
(04) 경험성 동사, 주체, 시간표현, 긍정적인 표현(좋음)을 사용하여 글을 작성해주세요.
예시) 최근에 브랜드 리뉴얼을 준비하면서 ${brand}과 함께했는데, 방향성이 명확해지고 결과도 좋아서 정말 잘한 선택이었다고 생각해요!

[제목 및 마무리 규칙]
(01) <u><strong>제목에 필수 키워드('${brand}')를 자연스럽게 맨 앞 또는 중간에 배치해주시고, 제목이 동사/질문형태로 끝나지 않게 부탁드립니다.</strong></u> ('${brand}' 키워드를 제목 내 필수로 넣어주세요)
<ul><li>틀린 예시) "${brand}로 ${main}을 시작한다면?"</li><li>맞는 예시) "${brand} ${main} 솔루션으로 만드는 매출 중심의 시장 진입 전략"</li></ul>(02) 맞춤법 검사기를 사용하여 검수하시고 업로드 해주시기 바랍니다.
(03) 좋아요와 블로그 스크랩을 열어놔 주세요.
(04) 블로그 최하단에 아래의 키워드를 순서대로 태그해주세요.
<ul><li>블로그 글 하단에는 키워드 태그하지 말아주세요.</li></ul><span style="background-color: #FFF2CC;">${hashtags}</span>
(05) 글 최하단에 쪽지로 전달드린 서비스 링크를 첨부해주세요. (리뷰 선정대상자분들께 쪽지 드릴 예정입니다.)
`;

  const section4 = `<strong>🚨 추가 안내</strong>

(01) 가이드라인이 지켜지지 않을 시 수정 요청이 있을 수 있습니다.`;

  let section4Full = section4 + `
(02) 협의 없이 캠페인 취소가 불가합니다.
(03) 작성하신 게시글은 6개월 이상 유지해주세요.`;

  return {
    section1: section1.replace(/\n/g, '<br>'),
    section2a: section2a.replace(/\n/g, '<br>'),
    section2b: section2b.replace(/\n/g, '<br>'),
    section3: section3.replace(/\n/g, '<br>'),
    section4: section4Full.replace(/\n/g, '<br>')
  };
}
...
function buildFullText(brand, main, sub, hashtags) {
  ...
  const photoSection = section2a + '<br>' + section2b;
  return [section1, photoSection, section3, section4].join('<br><br>');
}
...
form.addEventListener('submit', (e) => {
  ...
  // 각 섹션 렌더링
  resultSection1.innerHTML = section1;
  resultSection2a.innerHTML = section2a;
  resultSection2b.innerHTML = section2b;
  resultSection3.innerHTML = section3;
  resultSection4.innerHTML = section4;
...
copyBtn.addEventListener('click', async () => {
  const htmlText = copyBtn.dataset.fullText || '';

  // HTML에서 태그를 제거하여 순수 텍스트(Plain Text) 생성
