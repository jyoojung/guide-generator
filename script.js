...
[촬영 방법]
- 쪽지로 보내드린 예시 화면을 그대로 캡처하지 마시고, 서비스 소개서나 웹페이지의 다양한 화면을 컴퓨터, 휴대폰 화면으로 연 상태에서 촬영한 파일을 업로드 부탁드립니다.
- 원고 내에 사진들을 한 각도로(한 배경으로) 찍는 게 아닌 다양한 각도의 다른 배경으로 찍어서 작성 부탁드립니다.`;

  // 사진 가이드 — 이미지 슬롯 후
  const section2b = `
[첨부 및 배치 규칙]
- 사진 수 : 10 ~ 20장 이내로 첨부 부탁드립니다.
- 사진 활용 : [사진 1장] - [글 10줄 이내] - [사진 1장] - [글 10줄 이내] 의 형식으로 작성해주세요.
  → 사진 1장 당 사진 아래에 최소 글 4줄 ~ 5줄을 꼭 지켜주세요!
- 사진이 3장 이상 모여있거나 연속되지 않게, 글은 10줄 이상 넘지 않도록 작성해주세요.`;

  const section3 = `✒️ 내용 작성 가이드
[참고 자료]
- 보내드린 서비스 소개서 및 링크를 확인하여 작성하실 내용을 참고해주세요.

[키워드 작성 규칙]
- 원고에 주요 키워드 '${main}'은(는) 7번 이내로 작성해주세요.
- 다음 중 한 가지 키워드를 선택하고 해당 키워드는 7번 이내로 작성해주세요.
  → ${sub}
- 선택한 키워드를 제외한 다른 키워드는 3번 이상 중복하여 사용하지 말아주세요.
- 원고에 '${brand}'(은)는 1번 이내로 작성 부탁드립니다. (최대한 사진을 통한 간접 노출을 부탁드립니다.)

[원고 작성 방향]
- 다른 홈페이지, 커뮤니티, 기사 정보성 글을 그대로 복사해서 작성하지 마시고 정보성 글을 사용하게 된다면 글을 쓰는 방식을 많이 우회해서 작성 부탁드립니다.
- 히든 이미지 사용 불가한 점 양해 부탁드립니다.
- 원고 내에 ${brand}의 특장점 혹은 선택한 이유 등을 숫자를 붙여 작성해 자연스럽게 숫자가 들어가게 해주세요.
  → 예) 브랜드 성공을 위한 첫걸음, ${brand}를 선택한 이유
  1.~~
  2.~~
  3.~~
- 경험성 동사, 주체, 시간표현, 긍정적인 표현(좋음)을 사용하여 글을 작성해주세요.
  → 예) 최근에 브랜드 리뉴얼을 준비하면서 ${brand}과 함께했는데, 방향성이 명확해지고 결과도 좋아서 정말 잘한 선택이었다고 생각해요!

[제목 및 마무리 규칙]
- 제목에 필수 키워드('${brand}')를 자연스럽게 맨 앞 또는 중간에 배치해주시고, 제목이 동사/질문 형태로 끝나지 않게 부탁드립니다. ('${brand}' 키워드를 제목 내 필수로 넣어주세요)
  → 틀린 예) "${brand}로 ${main}을 시작한다면?"
  → 맞는 예) "${brand} ${main} 솔루션으로 만드는 매출 중심의 시장 진입 전략"
- 맞춤법 검사기를 사용하여 검수하시고 업로드 해주시기 바랍니다.
- 좋아요와 블로그 스크랩을 열어놔 주세요.
- 블로그 최하단에 아래의 키워드를 순서대로 태그해주세요.
  → 블로그 글 하단에는 키워드 태그하지 말아주세요.
  ${hashtags}
- 글 최하단에 쪽지로 전달드린 서비스 링크를 첨부해주세요.
  [여기에 서비스 링크를 삽입해주세요]`;

  const section4 = `🚨 추가 안내
- 가이드라인이 지켜지지 않을 시 수정 요청이 있을 수 있습니다.
- 협의 없이 캠페인 취소가 불가합니다.
- 작성하신 게시글은 6개월 이상 유지해주세요.`;
...
form.addEventListener('submit', (e) => {
  ...
  if (uploadedImageDataURL) {
    ...
    resultImageSlot.style.display = 'none';
  }
